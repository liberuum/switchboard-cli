...
### 5. Migrate a drive between profiles

```bash
switchboard migrate my-drive --from local --to staging
```

Preserves the drive's UUID, every document's UUID, and the full operation
history. Aborts if the destination already has a drive with the same slug.
...
From then on `docs apply`, `docs mutate`, `docs link`, `docs annotate` and `docs unlink` send actions through the typed `execute` mutation with a `context.signer`: ECDSA P-256 over `"\x19Signed Operation:\n" + len + timestamp + did + actionHash + prevStateHash`, where `actionHash` is the reactor's v2 hash — `"v2:" + base64url(sha256(canonicalJson([…])))` over the document, branch, scope, type, action id, timestamp, input and signer — exactly what `@powerhousedao/shared`'s `signActionV2` produces. The reactor stores the action untouched and refuses a tampered one. A v2 signature is bound to one action in one document: a document created by a 6.2.3-dev.24 or later reactor refuses anything else (`SCHEME_BELOW_POLICY`), and an older Switchboard, which checks only the ECDSA, accepts it too. `docs create` and the model-namespaced paths that build actions server-side remain server-signed.

...
The CLI automatically injects `timestampUtcMs` (as a JSON number, e.g. `1774218664353`) into each action if missing. This is required by the reactor's operation store (which does `new Date(timestampUtcMs)` — needs a number, not a string) but not populated by the generic `mutateDocument` resolver — without it, drive operations (ADD_FOLDER, MOVE_NODE, etc.) fail with "Invalid time value".
